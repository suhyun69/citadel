...
function untilFirstPick(seed: number, playerCount: number) {
  let s = createMatchUnchecked(matchConfig({ seed, playerCount }));
  while (!s.pending) s = step(s);
  return s;
}
...
describe('선택 단계', () => {
  ...
  it('4번 캐릭터는 절대로 앞면으로 버려지지 않는다', () => {
    for (let seed = 0; seed < 300; seed++) {
      ...
      for (const id of s.selection?.faceUp ?? []) {
  ...
  it('4번 캐릭터가 뒷면으로는 버려질 수 있다', () => {
    ...
    for (let seed = 0; seed < 300 && !sawRank4FaceDown; seed++) {
      ...
      sawRank4FaceDown = (s.selection?.faceDown ?? []).some(
  ...
  it('7인 게임의 마지막 픽커는 뒷면 버림까지 2장 중에 고른다', () => {
    let s = untilFirstPick(3, 7);
    const optionCounts: number[] = [];

    // 선택 단계가 끝날 때까지 랜덤봇 대신 항상 첫 선택지를 고른다.
    while (s.selection) {
      if (s.pending?.type === 'selectCharacter') {
        optionCounts.push(s.pending.options.length);
        const first = s.pending.options[0];
        expect(first).toBeDefined();
        s = applyChoice(s, { type: 'selectCharacter', characterId: first! });
      } else {
        s = step(s);
      }
    }
  ...
  it('선택이 끝나면 모든 플레이어가 캐릭터를 정확히 1장씩 가진다', async () => {
    ...
    const held = s.players.filter((p) => p.character !== null);
...
describe('게임 종료', () => {
  it('도시를 완성한 플레이어가 있어야 끝난다', async () => {
    ...
    expect(s.players.some((p) => p.city.length >= s.config.targetCitySize)).toBe(true);
    expect(s.firstCompleted).not.toBeNull();
  });

  it('최초 완성자는 4점, 그 외 완성자는 2점을 받는다', async () => {
    const s = await playRandomGame({ seed: 5, playerCount: 4 });
    for (const score of s.result!.scores) {
      const p = s.players[score.player]!;
      if (p.cityCompletedAtRound === null) expect(score.completionBonus).toBe(0);
      else if (s.firstCompleted === score.player) expect(score.completionBonus).toBe(4);
