...
M0~M5 완료. 기본 조합(캐릭터 8 + 특수 건물 14)으로 AI 봇 4~7명이 게임을 완주하고, 브라우저에서 관전할 수 있는 상태다.

| # | 내용 | 커밋 | 결과 |
|---|---|---|---|
| M0 | 코드젠 + `preset.md` | `b2d295f` | 검증 3종(오타·순번 완전성·종류)이 실제로 빌드를 깨뜨리는 것 확인 |
| M1 | 엔진 골격 + 랜덤봇 완주 | `24c1a90` | 1000판 완주, 불변식 위반 0, 평균 14.5라운드 |
| M2 | 캐릭터 8종 | `e1f2a8e` | 카드별 단위 테스트 21개 |
| M3 | 특수 건물 14종 + 점수 | `ba10b01` | `isPlayable('basic') === true` |
| M4 | 휴리스틱 봇 | `6b8f6d3` | 무작위 상대 99.5%, 미러 매치 26.5%(기대 25%) |
| M5 | 관전 UI | `a2fc588` | 브라우저 완주 + 일시정지/한 스텝/재생 |

테스트 78개, 타입·린트 통과.

## 설계대로 간 것

- **pending 결정 모델.** 엔진이 `state.pending` 에 멈추고 에이전트가 답하는 구조가 CLI·테스트·브라우저 세 경로에서 그대로 돌았다. `runMatch` 는 M1 이후 한 줄도 안 바뀌었다.
- **시드 결정론.** 퍼즈에서 깨진 판을 `--seed N --players M` 으로 즉시 재현했다. 실제로 두 번 그렇게 디버깅했다.
- **카드 총량 68 불변식.** 도적 소굴 자기 지불 버그를 이 그물이 잡았다.
- **훅 레지스트리.** 카드 22종이 코어 수정 없이 파일 추가만으로 들어갔다.

## 계획에서 바뀐 것

- **`useAbility` 에 식별자를 붙였다.** 장군(수입·파괴)과 상인(보너스·수입)처럼 한 캐릭터가 능력을 둘 가진다. `TurnState.abilityUsed` 불리언도 `usedAbilities` 배열로 바꿨다.
- **`ScoreCtx.wildcardAs` 를 추가했다.** 소원의 우물이 유령 지구의 종류 결정을 알아야 점수가 맞는다.
- **동명 건물 불변식을 약화했다.** 채석장으로 합법 건설한 뒤 장군이 채석장을 부수면 최종 상태만으로는 합법성을 증명할 수 없다. 카드 구성에 채석장이 아예 없는 경우로 범위를 좁혔다.
- **`legalChoices` 에서 상태 인자를 뺐다.** 결정에 실린 정보만으로 열거된다.

## 다음에 할 것

1. **사람 1인 투입** — `runtime/human-agent.ts` + `<DecisionPanel>`. `src/engine`·`src/bot`·`src/data` 변경 0줄이 판정 기준이다.
2. **추천 조합 6모드** — §11의 순서표대로. `preset.md` 에 행을 추가하고 `missingCards` 가 알려주는 카드를 구현한다.

## 알아둘 것

- `npm run sim -- --seed N --players M --verbose` 로 한 판을 텍스트로 본다.
- `npm run arena -- --a normal --b random` 로 봇 강도를 잰다. 좌석을 매판 돌리므로 미러 매치가 1/인원수 근처인지로 측정 자체를 검산할 수 있다.
- `npm run fuzz` 는 1000판(약 25초). CI 기본값은 240판.
- 브라우저 관전에서 "즉시" 속도는 `MessageChannel` 로 양보한다. `setTimeout(0)` 으로 되돌리면 백그라운드 탭에서 1초로 스로틀링되어 가장 느려진다.
